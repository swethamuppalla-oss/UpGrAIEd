  ...
  if (!activeDay) {
    return (
      <div style={{ textAlign: 'center', padding: 80 }}>
        ...
        <h2 style={{ fontSize: 24, marginTop: 24, color: 'var(--text-inverse)' }}>You finished this week's plan! 🎉</h2>
    ...
    <div>
      <div style={{ marginBottom: 32, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          ...
          <h1 style={{ fontSize: 28, margin: '8px 0', color: 'var(--text-inverse)' }}>{activeDay.title}</h1>
        </div>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          {plan.days.map((d, i) => (
            <div key={i} title={`Day ${d.day}: ${d.bloomLevel}`} style={{
              ...
              background: d.isComplete ? 'rgba(34,197,94,0.15)' : (d.day === activeDay.day ? 'var(--accent-primary)' : 'var(--bg-soft)'),
              color: d.isComplete ? 'var(--accent-green)' : (d.day === activeDay.day ? 'var(--text-inverse)' : 'var(--text-muted)'),
